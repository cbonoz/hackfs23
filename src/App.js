...
import OwnerLinks from "./components/OwnerBoard";
...
function App() {
  ...
  const changeNetwork = async (chainId) => {
    ...
    if (!e) {
      alert('Metamask must be connected to use FeatureChain')
      return
    }
      ...
      <Layout className="layout">
        <Header>
          <Menu
            ...
            mode="horizontal"
            selectedKeys={[path]}
            items={isBoard ? menuItems.filter(item => item.showOnBoardPage) : menuItems}
